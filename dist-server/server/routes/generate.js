...
import { generateSinglePdf, generateMultiplePdfs, } from "../../src/shared/pdf-generator.js";
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const upload = multer({ storage: multer.memoryStorage() });
export const generateRouter = Router();
generateRouter.post("/generate", upload.fields([
    ...
    try {
        ...
        // Resolve font
        let fontBytes;
        const fontName = req.body.fontName || AVAILABLE_FONTS[0].name;
        if (fontName === "Custom" && files.font?.[0]) {
            fontBytes = new Uint8Array(files.font[0].buffer);
        }
        else {
            const fontConfig = AVAILABLE_FONTS.find((f) => f.name === fontName) ||
                AVAILABLE_FONTS[0];
        ...
        if (outputFormat === "single") {
            ...
            res.setHeader("Content-Disposition", 'attachment; filename="winietki.pdf"');
            ...
            res.setHeader("Content-Disposition", 'attachment; filename="winietki.zip"');
            res.send(zipBuffer);
        }
    }
    catch (error) {
