...
import { findMatchingColumn, firstNameVariants, lastNameVariants, titleVariants, } from "./constants";
export function parseCsvString(csvContent) {
    ...
    return result.data
        ...
        return ((firstNameCol && row[firstNameCol]?.trim()) ||
        ...
        if (useDefaultPositions) {
            const columns = Object.values(row);
            return {
                firstName: (typeof columns[0] === "string" ? columns[0] : "").trim(),
                lastName: (columns.length > 1 && typeof columns[1] === "string"
                    ? columns[1]
                    : "").trim(),
                title: (columns.length > 2 && typeof columns[2] === "string"
                    ? columns[2]
                    : "").trim(),
            };
        }
        return {
            firstName: firstNameCol && typeof row[firstNameCol] === "string"
                ? row[firstNameCol].trim()
                : "",
            lastName: lastNameCol && typeof row[lastNameCol] === "string"
