...
export async function generateSinglePdf(options, onProgress) {
    ...
    const referenceTextWidth = font.widthOfTextAtSize(firstFullName, options.fontSize);
    ...
    for (let i = 0; i < options.persons.length; i++) {
        ...
        const currentTextWidth = font.widthOfTextAtSize(fullName, options.fontSize);
        let setX;
...
export async function generateMultiplePdfs(options, onProgress) {
    const templateDoc = await PDFDocument.load(options.templateBytes);
    const [templatePage] = await templateDoc.copyPages(templateDoc, [0]);
    const { width: pageWidth, height: pageHeight } = templatePage.getSize();
    ...
    const scaledY = pageHeight - options.positionY * pageHeight;
    const firstFullName = buildFullName(options.persons[0]);
    const referenceTextWidth = tempFont.widthOfTextAtSize(firstFullName, options.fontSize);
    const anchorX = scaledX + referenceTextWidth / 2;
    for (let i = 0; i < options.persons.length; i++) {
        ...
        const fileName = `${person.firstName}_${person.lastName}.pdf`.replace(/\s+/g, "_");
